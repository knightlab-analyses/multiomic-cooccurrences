setwd("~/Desktop/SEMultiomicsTest/reviseddepth_benchmarks/growth_sim")
...
microb <- read.table("rel.microbe.tsv",sep="\t",header=T,row.names = 1)
...
write.csv(ig.el.weight,"edgelist_rel_growth_0622_SEmultitest.csv")

sebeta <- as.matrix(symBeta(getOptBeta(se.test), mode='maxabs'))
rownames(sebeta) <- colnames(sebeta) <- nodenames
write.csv(sebeta,"weights_rel_growth_0622_SPIECEASI.csv")
dim(metab)
dim(microb)
...
write.csv(cor_result, "cor_matrix_rel_growth_0622_SparCC.csv")

comb<- rbind(metab,microb)
rho           <- propr(t(comb), metric = "rho")
rhoprop  <- rho@matrix

write.csv(rhoprop, "prop_matrix_rel_growth_0622_RHO.csv")

phi           <- propr(t(comb), metric = "phi")
phiprop  <- phi@matrix

write.csv(phiprop, "prop_matrix_rel_growth_0622_PHI.csv")
...
microb <- read.table("abs.microbe.tsv",sep="\t",header=T,row.names = 1)
...
ig.el<- as_edgelist(ig.se,names=TRUE)
ig.el.weight <- cbind(ig.el , round(E(ig.se)$weight,5 ))
write.csv(ig.el.weight,"edgelist_abs_growth_0622_SEmultitest.csv")

sebeta <- as.matrix(symBeta(getOptBeta(se.test), mode='maxabs'))
rownames(sebeta) <- colnames(sebeta) <- nodenames
write.csv(sebeta,"weights_abs_growth_0622_SPIECEASI.csv")
dim(metab)
dim(microb)
...
write.csv(cor_result, "cor_matrix_abs_growth_0622_SparCC.csv")

comb<- rbind(metab,microb)
rho           <- propr(t(comb), metric = "rho")
rhoprop  <- rho@matrix

write.csv(rhoprop, "prop_matrix_abs_growth_0622_RHO.csv")

phi           <- propr(t(comb), metric = "phi")
phiprop  <- phi@matrix

write.csv(phiprop, "prop_matrix_abs_growth_0622_PHI.csv")
